...
std::string pop(DynamicStack* stack) {
    if (stack->size == 0) {
        ...
        return ""; // Возвращаем пустую строку при попытке извлечь из пустого стека
    }
    std::string value = stack->data[stack->size - 1]; // Сохраняем значение
    stack->size--; // Уменьшаем размер
